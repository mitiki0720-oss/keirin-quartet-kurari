...
function renderMenu() {
  ...
  $view.innerHTML = `
    <section class="page menuPage">
      <div class="card card--menu">
        ...
        <div class="syncBox">
          <div class="syncTitle">データ同期（PC⇄スマホ）</div>

          <div class="syncBtns">
            <button id="syncExportBtn" type="button" class="syncBtn">📤 同期ZIPを書き出し</button>

            <input id="syncImportFile" type="file" accept=".zip" style="display:none;">
            <button id="syncImportBtn" type="button" class="syncBtn syncBtn--sub">📥 同期ZIPを取り込み</button>
          </div>

          <div class="syncHint">PCで書き出したZIPを、スマホに送って取り込みます。</div>
        </div>
  ...
  // --- Sync ZIP buttons ---
  const ex = document.getElementById("syncExportBtn");
  if(ex) ex.onclick = () => kqExportSyncZip();

  const imBtn = document.getElementById("syncImportBtn");
  const imFile = document.getElementById("syncImportFile");
  if(imBtn && imFile){
    imBtn.onclick = () => imFile.click();
    imFile.onchange = async () => {
      const f = imFile.files && imFile.files[0];
      if(!f) return;
      await kqImportSyncZip(f);
    };
  }
}
...
// =========================
// ✅ Sync ZIP (PC⇄スマホ) : Export / Import
// =========================

// localStorageのうち「同期対象」をまとめて拾う
function kqCollectLocalStorageForSync(){
  const out = {};
  const keep = (k) =>
    k.startsWith("kq_") ||
    k.startsWith("venueText:") ||
    k.startsWith("todayRace_") ||
    k.startsWith("today:") ||
    k === "today:lastDate";
...
// 同期対象のlocalStorageだけ掃除（他のキーは残す）
function kqClearLocalStorageForSync(){
  const remove = [];
  for(let i=0;i<localStorage.length;i++){
    const k = localStorage.key(i);
    if(!k) continue;
...
// IndexedDB（raceBundles）を全部吸い出す
async function kqIdbGetAllRaceBundles(){
  const db = await openKqDb();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(KQ_DB.store, "readonly");
    const store = tx.objectStore(KQ_DB.store);
...
// IndexedDB（raceBundles）を入れ替える（clear→put）
async function kqIdbReplaceAllRaceBundles(items){
  const db = await openKqDb();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(KQ_DB.store, "readwrite");
    const store = tx.objectStore(KQ_DB.store);
...
// 📤 同期ZIPを書き出す
async function kqExportSyncZip(){
  try{
    if(typeof JSZip === "undefined"){
      alert("JSZip が読み込めていません（ネット接続を確認）");
      return;
    }

    const meta = {
      schema: "KQ_SYNCZIP_V1",
      createdAt: new Date().toISOString(),
      note: "PC⇄スマホ同期ZIP（localStorage + IndexedDB raceBundles）"
    };

    const ls = kqCollectLocalStorageForSync();
    const bundles = await kqIdbGetAllRaceBundles();

    const zip = new JSZip();
    zip.file("meta.json", JSON.stringify(meta, null, 2));
    zip.file("localStorage.json", JSON.stringify(ls, null, 2));
    zip.file("raceBundles.json", JSON.stringify(bundles, null, 2));

    const ymd = new Date().toISOString().slice(0,10).replaceAll("-","");
    const hm  = new Date().toTimeString().slice(0,5).replace(":","");
    const filename = `KQ_SYNC_${ymd}_${hm}.zip`;

    const blob = await zip.generateAsync({ type: "blob", compression: "DEFLATE", compressionOptions:{ level: 6 } });
    downloadBlob(filename, blob);
  }catch(e){
    console.error(e);
    alert("同期ZIPの書き出しに失敗しました（console参照）");
  }
}

// 📥 同期ZIPを取り込む
async function kqImportSyncZip(file){
  try{
    if(!file){
      alert("ZIPファイルが選択されていません");
      return;
    }
    if(typeof JSZip === "undefined"){
      alert("JSZip が読み込めていません（ネット接続を確認）");
    ...
    if(!meta || meta.schema !== "KQ_SYNCZIP_V1"){
      alert("このZIPは同期ZIPではありません（KQ_SYNCZIP_V1 ではない）");
      return;
    }

    const lsRaw = await zip.file("localStorage.json")?.async("string");
    const dbRaw = await zip.file("raceBundles.json")?.async("string");

    const ls = lsRaw ? JSON.parse(lsRaw) : {};
    const bundles = dbRaw ? JSON.parse(dbRaw) : [];

    // 1) localStorage入れ替え
    kqClearLocalStorageForSync();
    ...
    // 2) IndexedDB入れ替え
    await kqIdbReplaceAllRaceBundles(Array.isArray(bundles) ? bundles : []);

    alert("✅ 同期ZIPを取り込みました。ページを更新します。");
    location.reload();
  }catch(e){
    console.error(e);
    alert("同期ZIPの取り込みに失敗しました（console参照）");
  }
}
